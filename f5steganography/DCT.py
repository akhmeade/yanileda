...
class DCT(object):
    ...
    def return_i_divisors(self, i, divisors, indata):
        return int(round(indata[i // 8][i % 8] * divisors))

    def quantize_block(self, indata, code):
        return [self.return_i_divisors(i, d, indata) for (i, d) in enumerate(self.divisors[code])]

    def return_i_quantum_extreme (i, quantum, indata):
        return int(round(indata[quantum[0] // 8][quantum[0] % 8] * quantum[1]))

    def return_i_quantum_extreme (self, i, quantum, indata):
        return int(round(indata[i // 8][i % 8] * quantum))

    def quantize_block_extreme(self, indata, code):
        return [self.return_i_quantum_extreme(i,d,indata) for (i,d) in enumerate(self.quantum[code])]
