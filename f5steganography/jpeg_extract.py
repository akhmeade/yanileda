...
class JpegExtract(object):
    ...
    def cal_embedded_length(self, permutation, coeff):
        ...
        while i < 32:
            ...
            if shuffled_index % 64 == 0:
                continue
            cc = coeff[shuffled_index - shuffled_index %
                       64 + self.de_zig_zag[shuffled_index % 64]]
            if cc == 0:
                continue
            elif cc > 0:
                extracted_bit = cc & 1
    ...
    def extract(self, data):
        ...
        logger.info('length of embedded file: %d bytes' %
                    self.extracted_file_length)
            ...
            try:
                while True:
                    ...
                    while code <= n:
                        ...
                        if shuffled_index % 64 == 0:
                            continue
                        shuffled_index = shuffled_index - shuffled_index % 64 + \
                            self.de_zig_zag[shuffled_index % 64]
                    ...
                    for i in range(k):
                        self.extracted_byte |= (
                            vhash >> i & 1) << self.available_extracted_bits
        ...
        if self.n_bytes_extracted != self.extracted_file_length:
            logger.info('incomplete file: only %d of %d bytes extracted' % (
                self.n_bytes_extracted, self.extracted_file_length))
